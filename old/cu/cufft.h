...
namespace cufft {
  ...
  template <typename Tin = cufftComplex, typename Tout = cufftComplex, unsigned DIM = 1> class FFT {
    public:
  ...
  template <> inline FFT<cufftComplex, cufftComplex, 1>::FFT(unsigned n, unsigned count)
  {
    plan = 0;
    checkCuFFTcall(cufftPlan1d(&plan, n, CUFFT_C2C, count));
  }

  template <> inline FFT<cufftComplex, cufftComplex, 1>::FFT(unsigned n, unsigned count, CUdeviceptr workArea, size_t workSize)
  {
  ...
  template <> inline FFT<std::complex<half>, std::complex<half>, 1>::FFT(unsigned n, unsigned count)
  {
    checkCuFFTcall(cufftCreate(&plan));

    long long size = n;
    size_t    neededWorkSize;
    checkCuFFTcall(cufftXtMakePlanMany(plan, 1, &size, nullptr, 1, 1, CUDA_C_16F, nullptr, 1, 1, CUDA_C_16F, count, &neededWorkSize, CUDA_C_16F));
  }
}
